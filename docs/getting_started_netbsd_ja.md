...
- **Xorg サーバー**: [Em68030-Guest-NetBSD リリース](https://github.com/hha0x617/Em68030-Guest-NetBSD/releases)の `xserver-wsfb-mvme68k-<hash>.tgz`、またはローカルビルドの `xserver-wsfb-mvme68k.tgz`
...
Xorg サーバーパッケージを CD-ROM 経由で転送して展開:
...
> **注**: ファイル名はローカルビルドでは `xserver-wsfb-mvme68k.tgz`、リリースダウンロードでは `xserver-wsfb-mvme68k-<hash>.tgz` です。ワイルドカード `*` でどちらにも対応します。
