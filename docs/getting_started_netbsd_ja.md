...
> **どちらを使うべき？** NetBSD をインストールしたい場合は **方法 B (コマンド
> ラインスクリプトに `-Miniroot` を指定)** を強くおすすめします。
> miniroot インストーラがあらかじめパーティション `b` に書き込まれた状態で
> イメージが生成されるため、フェーズ 1 を完全にスキップしてフェーズ 2
> (sysinst) から始められます。
>
> **方法 A (GUI の Create ボタン) は空のディスクイメージしか作成しません**。
> ディスクラベルは書き込まれますが miniroot は書き込まれません。方法 A を
> 選んだ場合は、下記フェーズ 1 で RAMDISK カーネルを起動し、自分で miniroot
> を書き込む必要があります。

### 方法 A: エミュレータ GUI を使用 (空のイメージ — フェーズ 1 が必要)
...
エミュレータは有効な NetBSD `cpu_disklabel` を書き込んだ **空** のディスク
イメージを作成します。パーティション `a` (ルートファイルシステム) と `b`
(スワップ、インストール時の miniroot 用にも使用) は定義されますが、
**miniroot 自体はこの方法では書き込まれません**。sysinst を起動するためには、
下記フェーズ 1 を実行して miniroot を書き込む必要があります。

### 方法 B: コマンドラインスクリプトを使用 (インストール用途で推奨)

`tools/create-netbsd-disk.ps1` スクリプトを使うと、コマンドラインから BSD
ディスクラベル付きのイメージを作成できます。`-Miniroot` で miniroot
イメージを渡すと、スクリプトがそれをパーティション `b` (sd0b) に直接
書き込むため、フェーズ 1 を経由せずに sysinst に進める状態のイメージが
得られます:
...
1. Em68030 を起動して **Settings** を開きます
2. SCSI Disks リストの **SCSI ID 0** に `netbsd.img` を追加します
3. **OK** をクリックし、**下記のフェーズ 1 はスキップ** して
   [フェーズ 2: ミニルート起動](#フェーズ-2-ミニルート起動--sysinst-で-netbsd-をインストール)
   に進みます

全オプションと Linux/WSL での使用方法は
[Disk Image and Utility Tools](tools_ja.md#netbsd-ディスクイメージの作成) を
参照してください。
...
> **このフェーズはスキップ可能**: 上記の方法 B
> (`create-netbsd-disk.ps1 -Miniroot miniroot.fs`) でディスクイメージを作成
> した場合、miniroot はすでにパーティション `b` に書き込まれているため、
> ここを飛ばして
> [フェーズ 2: ミニルート起動](#フェーズ-2-ミニルート起動--sysinst-で-netbsd-をインストール)
> に直接進んでください。
...
> **`10.0_2025Q4` パスについての注記**: このガイドは NetBSD 10.1 を対象としていますが、
> 上記のパスは意図的に `10.0_2025Q4` を指しています。NetBSD のパッケージミラーは現時点で
> NetBSD 10.0 と 10.1 の両方に対して同じ m68k バイナリを配信しており、`10.1/` パスは
> サーバ側で HTTP 302 リダイレクトにより `10.0_2025Q4/` に転送されます。NetBSD 10.x は
> メジャーリリース内でバイナリ互換性を保っているため、これらのパッケージは NetBSD 10.1
> 上でも正しく動作します。将来 10.1 専用のビルドが公開された場合は、この URL を更新
> してください。現在のディレクトリ一覧は
> `https://cdn.netbsd.org/pub/pkgsrc/packages/NetBSD/m68k/` で確認できます。
...
フレームバッファコンソールの設定および X Window System の設定については、
[フレームバッファディスプレイ セットアップガイド](https://github.com/hha0x617/Em68030-Guest-NetBSD/blob/main/docs/setup_framebuffer_ja.md) (Em68030-Guest-NetBSD) を参照してください。
