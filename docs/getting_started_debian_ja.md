...
### 1.2 スクリプトによる簡易セットアップ（推奨）

`tools/create-debian-disk.sh` スクリプトで手順 1.2〜1.6 を自動化できます:

```bash
sudo ./tools/create-debian-disk.sh -s 1G -p root -o debian.img
```

全オプションと必要なパッケージは [Disk Image and Utility Tools](tools_ja.md#debian-ディスクイメージの作成) を参照してください。

手動で設定する場合は、以下の手順に従ってください。

### 1.3 ディスクイメージの作成とパーティション分割（手動）
