...
約 270 MHz (JIT OFF, Avg モード実測)、約6.1 サイクル/命令
約 256 MHz (JIT ON, -5.1% オーバーヘッド)
...
> - アプリケーションレベルの性能は [ベンチマーク](benchmark_ja.md)（Dhrystone DMIPS）を参照。

> **STOP 命令アイドル時間の除外**: Linux は割り込み待ちに M68K の `STOP` 命令
> (0x4E72) を使用して CPU を停止させます（割り込み駆動型アイドル）。エミュレータは
> STOP 状態で経過した実時間を計測し、MHz 計算の分母から除外します。これにより
> 表示値はアイドル時間ではなく実際の CPU 実行速度を反映します。例えば、CPU がほぼ
> 全時間を STOP 状態で過ごす Linux のアイドルシェルプロンプトでも、MHz 値は
> 明らかな異常値を示すことなく、概ねアクティブ実行時の速度を維持します。一方 NetBSD はビジーウェイトループで命令を連続実行する傾向があるため、
> STOP 時間除外の影響は軽微です。
...
- サンプリング間隔 TickInterval (256)、コンパイル閾値 = 32 ヒット

#### チューニングパラメータ

以下のパラメータは 設定 → パフォーマンス → JIT で変更可能:

- **Compile Threshold**（デフォルト: 32）: 基本ブロックがコンパイルされるまでに必要な
...
- テスト: 772 件（JIT 専用テスト含む、CpuTests/JitCompilerTests.cpp）
...
|--------|----------------|------|
| JIT OFF | ~270 | ベースライン |
| JIT ON | ~256 | ベイルアウト頻度による -5.1% オーバーヘッド |
